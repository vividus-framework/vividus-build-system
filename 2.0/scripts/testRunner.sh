...
then
    ...
    java --add-exports=java.desktop/sun.awt.image=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED -Djava.security.manager=allow -cp $PATH_TO_ARTIFACT/classes:$PATH_TO_ARTIFACT/lib/* ${PROFILE:+ -Dprofile="$PROFILE"} ${ENCODING:+ -Dfile.encoding="$ENCODING"} $MAIN_CLASS
fi
